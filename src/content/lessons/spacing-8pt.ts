...
export const spacing8pt: Lesson = {
  ...
  exercises: [
    ...
    },
    {
      ...
      type: 'build',
      prompt:
        'Собери auto-layout карточку на холсте: отступ между блоками 16px, внутренние поля 24px. Держись 8pt-сетки.',
      blocks: 4,
      step: 4,
      min: 0,
      max: 40,
      ...
      explanation:
        'Отступ 16px (8×2) задаёт ровный вертикальный ритм между блоками, а поля 24px (8×3) дают карточке «дышащие» границы. Оба значения — на 8pt-сетке, поэтому композиция выглядит аккуратно.',
    },
    {
      id: 'sp-align-1',
      type: 'align',
      prompt:
        'Перетащи карточку так, чтобы она встала по центру фрейма. Она магнитится к направляющим и шагу 8pt.',
      target: { x: 'center', y: 'middle' },
      explanation:
        'Выравнивание по осям и привязка к сетке — основа опрятной композиции. Направляющие (левая/центр/правая, верх/центр/низ) — те же, что даёт Auto Layout: элементы встают предсказуемо, а не «примерно там».',
    },
  ],
  masteryChallenge: {
    id: 'sp-tune-1',
